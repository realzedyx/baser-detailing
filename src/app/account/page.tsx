...
const MOCK = {
  ...
  points: 340,
  bookings: [] as { id: string; date: string; service: string; amount: number; status: string }[],
  car: null as { make: string; model: string; year: number; colour: string } | null,
};
...
// ── Traveling light beam (matches signin aesthetic) ───────────────────────
function LightBeam({ delay = 0 }: { delay?: number }) {
  return (
    <div className="absolute -inset-[1px] rounded-2xl overflow-hidden pointer-events-none">
      <motion.div
        className="absolute top-0 left-0 h-[2px] w-[45%] bg-gradient-to-r from-transparent via-[#CBA65C] to-transparent"
        style={{ opacity: 0.5 }}
        animate={{ left: ['-45%', '100%'] }}
        transition={{ duration: 3, ease: 'easeInOut', repeat: Infinity, repeatDelay: 3, delay }}
      />
      <motion.div
        className="absolute top-0 right-0 h-[45%] w-[2px] bg-gradient-to-b from-transparent via-[#CBA65C] to-transparent"
        style={{ opacity: 0.5 }}
        animate={{ top: ['-45%', '100%'] }}
        transition={{ duration: 3, ease: 'easeInOut', repeat: Infinity, repeatDelay: 3, delay: delay + 0.75 }}
      />
      <motion.div
        className="absolute bottom-0 right-0 h-[2px] w-[45%] bg-gradient-to-l from-transparent via-[#CBA65C] to-transparent"
        style={{ opacity: 0.5 }}
        animate={{ right: ['-45%', '100%'] }}
        transition={{ duration: 3, ease: 'easeInOut', repeat: Infinity, repeatDelay: 3, delay: delay + 1.5 }}
      />
      <motion.div
        className="absolute bottom-0 left-0 h-[45%] w-[2px] bg-gradient-to-t from-transparent via-[#CBA65C] to-transparent"
        style={{ opacity: 0.5 }}
        animate={{ bottom: ['-45%', '100%'] }}
        transition={{ duration: 3, ease: 'easeInOut', repeat: Infinity, repeatDelay: 3, delay: delay + 2.25 }}
      />
    </div>
  );
}
    ...
    <div className={`relative group ${className}`}>
      {glow && (
        <motion.div
          ...
          style={{
            background: 'radial-gradient(ellipse at 50% 0%, rgba(203,166,92,0.2), transparent 70%)',
            filter: 'blur(10px)',
          }}
        />
      )}
      ...
      <div className="relative bg-black/40 backdrop-blur-xl rounded-2xl border border-white/[0.06] overflow-hidden shadow-2xl">
        {/* Grid texture — same as signin card */}
        <div
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 208, height: 208 }}>
      <div
        ...
        style={{ background: 'radial-gradient(circle at center, rgba(203,166,92,0.07) 0%, transparent 65%)', filter: 'blur(8px)' }}
      />
      ...
      <div className="absolute flex flex-col items-center">
        <motion.span
          ...
          style={{ fontSize: 44, color: '#CBA65C', fontWeight: 200, lineHeight: 1, letterSpacing: '-0.02em' }}
        >
          {pts}
        </motion.span>
        <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', letterSpacing: '0.28em', textTransform: 'uppercase', marginTop: 4 }}>
          points
        </span>
      </div>
    </div>
...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <motion.button
      ...
      className="text-xs tracking-widest transition-all duration-300 px-4 py-2 rounded-xl border"
      style={{
        borderColor: copied ? 'rgba(203,166,92,0.5)' : 'rgba(255,255,255,0.08)',
        background: copied ? 'rgba(203,166,92,0.08)' : 'transparent',
        color: copied ? '#CBA65C' : 'rgba(255,255,255,0.4)',
        cursor: 'pointer',
        fontSize: 11,
        letterSpacing: '0.1em',
      }}
...
export default function AccountPage() {
  ...
  return (
    <div className="min-h-screen w-screen relative overflow-x-hidden" style={{ backgroundColor: '#0a0a0a' }}>
      ...
      {/* Background — same as signin/signup */}
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div
          ...
          style={{ position: 'absolute', width: 500, height: 500, top: '2%', right: '-8%', background: 'radial-gradient(circle, rgba(203,166,92,0.055) 0%, transparent 70%)', filter: 'blur(50px)', borderRadius: '50%' }}
        />
        <motion.div
          animate={{ x: [0, -30, 0], y: [0, 40, 0] }}
          transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut', delay: 4 }}
          style={{ position: 'absolute', width: 600, height: 600, bottom: '5%', left: '-12%', background: 'radial-gradient(circle, rgba(203,166,92,0.04) 0%, transparent 70%)', filter: 'blur(60px)', borderRadius: '50%' }}
        />
      </div>
      ...
      <div className="relative z-10 mx-auto pb-32" style={{ maxWidth: 980, padding: '0 24px' }}>
        ...
        <motion.div {...s(0)} style={{ paddingTop: 72, marginBottom: 36 }}>
          ...
          <h1 style={{
            ...
            lineHeight: 1.0,
            margin: 0,
            background: 'linear-gradient(135deg, #E8E8E8 60%, rgba(203,166,92,0.7))',
            ...
            <motion.div {...s(2)}>
              <GlassCard glow beamDelay={0.2}>
                <div style={{ padding: '28px 28px 24px' }}>
                  <p style={{ fontSize: 10, color: '#CBA65C', letterSpacing: '0.24em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 24 }}>
                    Rewards
                  </p>

                  <div className="flex flex-col items-center" style={{ marginBottom: 24 }}>
                    <Ring pts={user.points} />
                    {nextTier ? (
                      <motion.p
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ delay: 1.2 }}
                        style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginTop: 14, letterSpacing: '0.14em', textTransform: 'uppercase' }}
                      >
                        {nextTier.pts - user.points} pts to {nextTier.name}
                      </motion.p>
                    ) : (
                      <p style={{ fontSize: 11, color: '#CBA65C', marginTop: 14, letterSpacing: '0.14em', textTransform: 'uppercase' }}>
                        VIP Member ✦
                      </p>
                    )}
                  </div>
                      ...
                      return (
                        <motion.div
                          ...
                          style={{
                            ...
                            padding: '11px 14px',
                            borderRadius: 12,
                            border: `1px solid ${active ? 'rgba(203,166,92,0.38)' : 'rgba(255,255,255,0.05)'}`,
                            background: active ? 'rgba(203,166,92,0.07)' : 'rgba(255,255,255,0.015)',
                            transition: 'all 0.3s',
                          }}
                        >
            ...
            <motion.div {...s(4)}>
              <GlassCard beamDelay={1.2}>
                <div style={{ padding: '24px 28px' }}>
                  <p style={{ fontSize: 10, color: '#CBA65C', letterSpacing: '0.24em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 18 }}>
                    My Car
                  </p>
                  {!user.car ? (
                    <div style={{ textAlign: 'center', padding: '24px 0' }}>
                      ...
                      <motion.button
                        ...
                        style={{ fontSize: 11, color: '#CBA65C', background: 'rgba(203,166,92,0.07)', border: '1px solid rgba(203,166,92,0.25)', borderRadius: 10, padding: '9px 22px', cursor: 'pointer', letterSpacing: '0.1em', textTransform: 'uppercase' }}
                      >
                        + Add your car
                      </motion.button>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                        <div style={{ width: 42, height: 42, borderRadius: 10, background: user.car.colour, border: '1px solid rgba(255,255,255,0.1)', flexShrink: 0 }} />
                        <div>
                          <div style={{ fontSize: 14, color: '#E8E8E8', fontWeight: 300 }}>{user.car.year} {user.car.make} {user.car.model}</div>
                          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 2, textTransform: 'capitalize' }}>{user.car.colour}</div>
                        </div>
                      </div>
                      <motion.button
                        whileHover={{ scale: 1.05 }}
                        style={{ fontSize: 11, color: '#CBA65C', background: 'none', border: '1px solid rgba(203,166,92,0.25)', borderRadius: 8, padding: '7px 16px', cursor: 'pointer', letterSpacing: '0.06em' }}
                      >
                        Edit
                      </motion.button>
                    </div>
          ...
          <div className="flex flex-col gap-5">
              ...
              <GlassCard beamDelay={0.6}>
                <div style={{ padding: '24px 28px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
                    <p style={{ fontSize: 10, color: '#CBA65C', letterSpacing: '0.24em', textTransform: 'uppercase', fontWeight: 500 }}>
                      Your Bookings
                    </p>
                    <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.18)', letterSpacing: '0.08em' }}>{user.bookings.length} total</span>
                  </div>
                  ...
                  {user.bookings.length === 0 ? (
                    <div style={{ padding: '36px 0', textAlign: 'center' }}>
                      <motion.div
                        ...
                        style={{ width: 44, height: 44, borderRadius: '50%', border: '1px dashed rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', fontSize: 14, color: 'rgba(255,255,255,0.15)' }}
                      >
                        ✦
                      </motion.div>
            ...
            <motion.div {...s(5)}>
              <GlassCard glow beamDelay={0.9}>
                <div style={{ padding: '24px 28px' }}>
                  <p style={{ fontSize: 10, color: '#CBA65C', letterSpacing: '0.24em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 10 }}>
                    Refer a Mate
                  </p>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <a
                      href={`sms:?body=${encodeURIComponent(referralMsg)}`}
                      style={{ fontSize: 11, color: 'rgba(255,255,255,0.32)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '8px 16px', textDecoration: 'none', display: 'flex', alignItems: 'center', letterSpacing: '0.06em' }}
                    >
                      Send as text
                    </a>
                  </div>
              ...
              <GlassCard beamDelay={1.5}>
                <div style={{ padding: '24px 28px' }}>
                  ...
                  <div style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: 16, marginBottom: 16 }}>
                    ...
                    <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', fontWeight: 300 }}>{user.email}</div>
                  </div>
                  <motion.button
                    whileHover={{ x: 3, color: 'rgba(192,57,43,0.9)' }}
                    whileTap={{ scale: 0.97 }}
                    onClick={() => {/* supabase.auth.signOut() + redirect */}}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50" style={{ padding: '20px 24px 36px', background: 'linear-gradient(to top, rgba(10,10,10,0.98) 55%, transparent)' }}>
        <div style={{ maxWidth: 980, margin: '0 auto' }}>
          <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}>
            <Link
              href="/book"
              className="block text-center rounded-2xl font-medium relative overflow-hidden"
              style={{
                background: 'linear-gradient(120deg, #BF9A50, #CBA65C 35%, #E4C883 60%, #CBA65C)',
                color: '#0a0a0a',
                textDecoration: 'none',
                padding: '15px',
